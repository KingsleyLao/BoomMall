...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 用户管理 <span class="c-gray en">&gt;</span> 用户列表 (<?php echo ($status==0?'白名单':'黑名单'); ?>)
<a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="pd-20" style="padding-top:0px">
	
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a href="/BoomMall v1/Boommall/Admin/Users/usersList/status/<?php echo ($status==0?1:0); ?>" class="btn <?php echo ($status==0?'btn-default':'btn-primary'); ?> radius"> <?php echo ($status==0?'进入黑名单':'进入白名单'); ?></a></span> 

	<div class="text-c r"> 
		<form action="/BoomMall v1/Boommall/Admin/Users/usersList/status/<?php echo ($status); ?>" method="post">
		<input type="text" class="input-text" style="width:250px" placeholder="输入用户名称" id="" name="search" value="<?php echo ($search); ?>">
	...
	<table class="table table-border table-bordered table-bg">
		...
		<tbody>
			...
			<?php if($usersInfo == null): ?><tr class="text-c">
					<td colspan="8">暂无数据</td>
				</tr>

			<?php else: ?>
			<?php if(is_array($usersInfo)): foreach($usersInfo as $key=>$v): ?><tr class="text-c">
				...
				<td class="td-status"><span class="label <?php echo ($v['status'] == 0 ? 'label-success':'label-default'); ?> radius"><?php echo ($v['status'] == 0? '已启用':'黑名单'); ?></span></td>
				

				<td class="td-manage">
					
					
					<a style="text-decoration:none" onClick="<?php echo ($v['status'] == 0? 'admin_stop':'admin_start'); ?>(this,'<?php echo ($v["id"]); ?>')" href="javascript:;" title="<?php echo ($v['status'] == 1? '启用':'停用'); ?>"><i class="Hui-iconfont"><?php echo ($v['status'] == 0? '&#xe631':'&#xe615'); ?>;</i></a> 			
...
function admin_stop(obj,id){
	layer.confirm('确认要停用吗？',function(index){
		...
		$.get('<?php echo U('Users/changeStatus');?>','status=1&id='+id);
...
function admin_start(obj,id){
	layer.confirm('确认要启用吗？',function(index){
		...
		$.get('<?php echo U('Users/changeStatus');?>','status=0&id='+id);
